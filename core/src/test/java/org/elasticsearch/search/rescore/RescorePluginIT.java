...
public class RescorePluginIT extends ESIntegTestCase {
    ...
    public void testRescorePlugin() throws IOException {
        ...
        assertThat(hits.hits().length, equalTo(2));
        assertThat(hits.maxScore(), equalTo(4.0f));
        ...
        logger.info("Hit[0]: {}, Score: {},\nExplanation: {}", hit.getId(), hit.getScore(), hit.explanation());
        assertThat(hit.getScore(), equalTo(4.0f));
        assertThat(hit.explanation().getDetails()[1].toString(), equalTo("4.0 = weight\n"));
        hit = hits.getAt(1);
    ...
    public static class CustomRescorePlugin extends Plugin implements SearchPlugin {
        ...
        public List<RescoreSpec<?>> getRescorers() {
            return singletonList(new RescoreSpec(
                    DummyRescorerBuilder.NAME, DummyRescorerBuilder::new, DummyRescorerBuilder.PARSER));
        }
    }
    ...
    public static class DummyRescorerParser implements RescoreParser<DummyRescorerBuilder> {
        ...
        @Override
        public DummyRescorerBuilder fromXContent(QueryParseContext parseContext) throws IOException, ParsingException {
            Float weight = null;
            ...
            while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                if (token == XContentParser.Token.FIELD_NAME) {
                    ...
                    if (parseContext.getParseFieldMatcher().match(fieldName, WEIGHT_FIELD)) {
                        weight = parser.floatValue();
                    } else {
                        throw new ParsingException(parser.getTokenLocation(), "dummy rescorer doesn't support [" + fieldName + "]");
                    }
                }
            }

            if (weight == null) {
                throw new ParsingException(parser.getTokenLocation(), "dummy rescorer requires [weight] field");
            }
